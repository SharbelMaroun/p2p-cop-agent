"""Drive the passive mailbox, so a peer can play a match unattended (M5-17).

``adapters.build_server`` is a *mailbox*: its four tools enqueue the opponent's
message and acknowledge it, and nothing inside them advances the game.
``run_turn`` needs the opposite -- a ``Receive`` that yields the opponent's next
turn or gives up. This module is that bridge, and it is what turns a harness fed
by hand into a peer that plays on its own.

**Poll to drain; the state machine still decides.** The reference drives its
runtime by polling its *own* inboxes at ``[network].poll_interval_seconds``
(0.5 s in the shipped config), while the book mandates a strict state machine
gating every transition (§8.3). Both hold here, because they answer different
questions: polling is only *how* a queued message is picked up, and
:class:`~p2p_cop_agent.orchestration.phases.PhaseMachine` still decides what may
legally happen next. A message that arrives out of turn is refused by the
transition table, never by this loop.

**Silence is bounded, never patient.** Appendix E rule 6 makes a deadline
mandatory "to prevent deadlocks while waiting for the opponent", so the wait
stops at the turn timeout and returns ``None`` -- which ``run_turn`` turns into
the one declared exit to ``TECHNICAL_LOSS``. Blocking forever is precisely the
failure this exists to prevent `[AE-6]`.
...
asleep inside this loop still proves it is alive `[AE-7]`.
...
def poll_for_turn(
    ...
    *waiting*, and refusing a message that has already arrived would forfeit a
    ...
    The boundary itself counts as expired, matching
    :mod:`~p2p_cop_agent.services.deadlines` -- one convention for every
    time-bound in the peer, so "expired" never means two different things.
    """
    if poll_interval <= 0:
...
def turn_receiver(
    ...
    *one* turn; carrying a deadline across turns would make a fast opponent
    subsidise a slow one and eventually expire a turn that never got its own
    full allowance.
    """
