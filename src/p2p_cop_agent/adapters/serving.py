"""Host this peer's mailbox on a background thread, reachable through a tunnel (M5-17e).
...
the main thread; this module does the same `[ADR-009]`.
...
from p2p_cop_agent.adapters.fastmcp_server import PeerInboxes, build_server
...
def ensure_port_free(host: str, port: int) -> None:
    ...
    try:
        ...
        raise ServingError(
            f"cannot host the Cop mailbox on {host}:{port}: {exc}. "
...
def serve_in_background(
    ...
    *opponent* is a separate concern (:mod:`p2p_cop_agent.services.readiness`).
    """
...
#
# 530 added 2026-08-12: Cloudflare returns it (with body "error code: 1033") when the
# hostname exists but NO tunnel is registered for it -- the opponent's route is not up.
# Without it here, dialling a 530 endpoint passed readiness on the first probe and then
# failed 8s later inside negotiate, instead of the readiness loop waiting out its budget
# for the peer to come online. It is the same "no origin" condition as 502, one layer up.
...
def peer_answers(url: str, timeout: float = 2.0) -> bool:
    ...
    **`port_answers` cannot answer this through a tunnel, and that cost us a match.**
    ...
    try:
        ...
        # 403 (bot protection), which is not a `_NO_ORIGIN` status, so readiness read every
        # tunnelled peer as "up" and never waited -- the 8s failure on game 2 (2026-08-12).
        # curl saw the true 502/530; urllib did not, until it identified itself.
        request = urllib.request.Request(
            url, method="GET", headers={"User-Agent": "p2p-cop/1.0 (readiness probe)"})
