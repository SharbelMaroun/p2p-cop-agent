...
**Locking is enforced, not indicated (`M8-01c`).** Asked directly, the interface "enforces
the lock" after the commit is sent to prevent a race where both sides act on the same turn,
and the row's condition is that out-of-turn input is "ignored, not queued" — so the buttons
are genuinely disabled rather than dimmed, and a click that lands during the transition is
dropped by `_guarded` rather than remembered.
...
class LiveWindow:
    ...
    def _guarded(self, move: str) -> Callable[[], None]:
        ...
        Disabling the buttons already prevents most of this; the guard covers the moment
        between a commit being sent and the repaint landing. A queued move would surface
        one turn later as an action nobody chose.
        """
        def handler() -> None:
