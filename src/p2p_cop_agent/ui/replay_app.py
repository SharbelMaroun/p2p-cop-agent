...
forth in time" (p.56/141). **The board is drawn as well (`M8-15`)**: the replay runs in
the audit phase as the book's "Retrospective Witness" (p.54/135) — rule 9's objective-board
ban binds the live interface only — and the reference paints both true positions on one
board when the opponent's log is beside our own, falling back to a single trail when it is
not. `Play` auto-advances the cursor so the whole chase reads at a glance; the banner
remains the mandatory screenshot either way, its green/red **semantic and pinned** — only
the chrome around it (`ui/style.py`) is styling.

**Every widget reads `ReplayFrame` and nothing else** (`M8-06`: "no widget touches domain
or protocol code directly"). Excluded from coverage by `M8-06c`; what *can* be asserted
lives in `replay/view_model.py` and `replay/board.py`, whose tests pin the banner text,
the colours, and the board data this window draws.
"""
...
from p2p_cop_agent.replay.view_model import frame_of
from p2p_cop_agent.ui.replay_board import paint_board
from p2p_cop_agent.ui.replay_panels import (
    BAD_TEXT,
    build_detail,
    paint_detail,
    paint_rows,
...
class ReplayWindow:
    ...
    def _build(self) -> None:
        ...
        self._detail_labels = build_detail(detail, self._mono)
    ...
    def _toggle_play(self) -> None:
        """Auto-advance the cursor; stop at the last step or on `Pause` (`M8-15`)."""
    ...
    def refresh(self) -> None:
        ...
        paint_detail(self._detail_labels, frame)
