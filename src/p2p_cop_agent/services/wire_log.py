"""Record what actually arrived on the wire, because the access log does not (`M9-31`).

**Why this exists.** On 2026-08-11 an offer from group `uoh-ay26` reached the companion
Thief and vanished. All we had afterwards was uvicorn's access log — a column of `200 OK` — which is
...
def delivery(result: Any) -> Any:
    """Record a `Delivery`'s verdict and hand it straight back.

    Pass-through so the caller stays one line: `return wire_log.delivery(Delivery(...))`.
    `adapters/fastmcp_server.py` sits exactly on the 150-line gate (`G-04`), and a
    diagnostic is not worth pushing a protocol module over it.
    """
    validated(str(getattr(result, "tool", "?")),
