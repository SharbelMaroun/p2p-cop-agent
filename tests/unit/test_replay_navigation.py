"""`M8-08` / `M8-08a`: stepping through a replay, and never trusting a stale verdict.

`:1689` asks for navigation "forward and backward in time using playback controls".
`M8-08a` adds the condition that matters: "the verdict is derived, never cached from load
time". The test that proves it is `test_tampering_between_two_navigations_flips_the_banner`
— everything else here is a precondition for that one being meaningful.
"""
...
from p2p_cop_agent.replay import Replay, Verdict, parse_log
...
def _record(step: int) -> dict:
    ...
    return {"step": step, "sender": "thief", "move": "E", "payload": payload,
            "nonce": NONCE, "commit": _commit(payload)}
...
def test_the_cursor_clamps_at_both_ends_instead_of_raising() -> None:
    """A `Next` button that throws at the last step crashes during the demo it exists to
    produce — and the demo is the submission evidence."""
    replay = _replay(3)
    for _ in range(10):
        replay.step_forward()
...
def test_go_to_first_divergence_lands_on_the_forged_step() -> None:
    """The only navigation an auditor actually performs — `:1769` has already decided the
    match, so the remaining question is *which step*."""
    replay = _replay(6)
...
# --- M8-08a: the verdict is recomputed, not remembered ----------------------------------
...
def test_tampering_between_two_navigations_flips_the_banner() -> None:
    ...
    has to track the bytes it is describing at the moment it is read.

    No reload happens here — the same `Replay` object reports green, then red.
    """
    replay = _replay(4)
...
def test_repairing_a_log_flips_the_banner_back() -> None:
    """The other direction, which a cached verdict would also get wrong — and which proves
    the flip above is recomputation rather than a latch that only ever trips one way."""
    replay = _replay(3)
...
def test_the_replay_object_stores_no_verdict_to_go_stale() -> None:
    ...
    replay = _replay()
    stored = vars(replay)
    assert set(stored) == {"_log", "_position"}


def test_the_per_step_check_is_recomputed_too() -> None:
    """A cached *record* verdict would be the same bug one level down."""
    replay = _replay(3)
    replay.go_to(1)
