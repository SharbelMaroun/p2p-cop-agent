"""`M9-23`: the readout must be able to say NOT ready, or it is decoration.
...
from p2p_cop_agent.services.preflight import ARMED, DISABLED, preflight

ROOT = Path(__file__).resolve().parents[2]
MATCH = ROOT / "shared_contract" / "fixtures" / "match_config.example.json"
...
def _private(tmp_path: Path, *, port: int = 8899, credential: str = "C:/nowhere/token.json",
             group_id: str = "neutral-group-alpha") -> Path:
...
def test_a_match_naming_roles_instead_of_groups_fails(tmp_path: Path) -> None:
    ...
    assert check.failed
    assert "neutral-group-alpha" in check.value
...
def test_an_unsupported_schema_version_fails(tmp_path: Path) -> None:
    """`ADR-003`: uoh-ay26 sent `"1.00"`, the reference ships `"1.3"`, Appendix B says `"1.2"`.
