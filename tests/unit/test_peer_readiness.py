"""`M9-26`: readiness must mean *served*, not merely *routable*.

**Found in a live match attempt on 2026-08-09, not by any test.** `serve_match` waits up to
...
from p2p_cop_agent.adapters.serving import peer_answers, port_answers
...
def test_a_gateway_error_means_the_peer_is_not_up(status: int) -> None:
    ...
    not there, however routable its hostname is. 530 is Cloudflare's "no tunnel registered"
    (error 1033); added 2026-08-12 after dialling a 530 endpoint passed readiness and then
    failed inside negotiate 8s later, instead of the wait running its course."""
