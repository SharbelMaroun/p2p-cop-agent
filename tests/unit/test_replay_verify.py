"""`M8-02` / `M8-02c`: the two verdicts, and the one that voids the match.
...
project. The binding assertions are that a single altered step condemns the whole match
(`:1753`) and that there is no third verdict to hide in (`:1769`: "no appeal process").

Which hash construction is authoritative is `test_replay_authority.py` (`M8-02d`).
"""
...
from p2p_cop_agent.replay import Verdict, verify_record, verify_records
...
def _commit(payload: object, nonce: str = NONCE) -> str:
    ...
    return hashlib.sha256(canonical + b"|" + nonce.encode()).hexdigest()


def _record(step: int, move: str = "N", nonce: str = NONCE) -> dict:
    payload = {"step": step, "move": move, "intent": True}
    return {"step": step, "sender": "police", "move": move, "hint": "north",
            "intent": True, "payload": payload, "nonce": nonce,
            "commit": _commit(payload, nonce)}


def _log(count: int = 4) -> dict:
    return {"_schema": "game-log", "game_id": "demo", "sub_game_number": 1,
            "records": [_record(n) for n in range(1, count + 1)]}
...
def test_an_untouched_log_verifies_and_says_how_many_steps() -> None:
    result = verify_records(_log(4)["records"])
...
def test_verification_recomputes_rather_than_trusting_the_stored_commit() -> None:
    """The commits above were built by this test's own hashing expression, which imports
    nothing from the commit module. If the two constructions ever drift, this fails."""
    assert verify_record(_record(1)).ok


# --- M8-02c: one bad step voids the whole match -----------------------------------------
...
def test_the_steps_after_a_forgery_are_still_checked_and_still_pass() -> None:
    ...
    result = verify_records(records)
    assert [check.ok for check in result.checks] == [True, False, True, True, True]


def test_a_forgery_in_the_very_last_step_is_caught_too() -> None:
    """A verifier that stopped early — or sampled — would pass this log."""
    records = _log(6)["records"]
...
def test_a_record_stripped_of_a_verification_input_is_tampered(field: str) -> None:
    """Damaging a record must not be an easier escape than rewriting one. The reference
    collapses its `CryptoError` to the same red banner."""
    record = _record(1)
    del record[field]
...
def test_an_empty_record_list_is_tampered_not_vacuously_verified() -> None:
    """The dangerous default: a verifier that returns OK for zero records would stamp
    `Verified OK` on an emptied log, which is the easiest forgery of all."""
...
def test_a_payload_that_is_not_an_object_still_verifies_by_its_digest() -> None:
    """An opponent may seal an array or a bare string rather than an object. The digest is
    what binds it, so that log verifies; there are simply no named fields to cross-check
    against the visible ones. Refusing it would fail rule 36 on a shape preference.
    """
    payload = ["N", 1, True]
