"""`M9-07b`: the charts encode their data, and say so in words as well as colour.
...
caption `M9-07b` asks for is present, and that no chart depends on colour alone.
"""
...
from p2p_cop_agent.analysis import (
    Series,
    bar_chart,
    box_plot,
    heatmap,
...
def test_every_chart_carries_its_title_caption_and_both_axis_labels() -> None:
    """`M9-07b`'s literal condition: "clear axes, legend, caption"."""
    charts = (
...
def test_a_line_chart_marks_each_measured_point_rather_than_only_joining_them() -> None:
    """A smooth line hides how many measurements there were. `M9-06c` cares about run
