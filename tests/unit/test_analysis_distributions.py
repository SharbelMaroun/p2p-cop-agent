"""`M9-07b`: the two chart types §9.3 names for distributions and sensitivity.
...
box plot must survive a zero-variance group — the oracle arm scores 20 on every seed, which
...
from p2p_cop_agent.analysis import box_plot, heat_cell_colour, heatmap, summarise
...
def test_a_zero_variance_group_still_draws_a_visible_box() -> None:
    """The oracle arm scores 20 on every seed — a real result, and one that would vanish if
    the box height came straight from `q3 - q1`."""
    svg = box_plot(**COMMON, labels=["oracle"], summaries=[summarise([20.0] * 5)])
