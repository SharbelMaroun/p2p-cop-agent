"""M5-17e: hosting the mailbox -- bind address, daemon thread, port pre-check.
...
from p2p_cop_agent.adapters import PeerInboxes
from p2p_cop_agent.adapters.serving import (
    DEFAULT_BIND_HOST,
    ServingError,
    ensure_port_free,
    port_answers,
...
def test_a_taken_port_fails_loudly_at_launch() -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as held:
        ...
        with pytest.raises(ServingError, match="cannot host the Cop mailbox"):
...
def test_the_thread_is_named_so_a_stuck_peer_is_identifiable() -> None:
    thread = serve_in_background(
        PeerInboxes(), port=a_free_port(), host="127.0.0.1", name="p2p-cop"
    )
    assert "p2p-cop" in thread.name
