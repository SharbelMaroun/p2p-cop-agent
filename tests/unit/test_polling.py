"""M5-17: the polling turn source that lets a peer play a match unattended.

The mailbox (``adapters.build_server``) only enqueues; ``run_turn`` only consumes.
Nothing joined them, which is why a full game needed a harness feeding messages
in by hand. These tests pin the join.
...
from p2p_cop_agent.orchestration.polling import PollingError, poll_for_turn
