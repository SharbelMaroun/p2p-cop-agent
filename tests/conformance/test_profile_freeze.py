"""`M8-14`: pin the wire surface, so a change after the league starts cannot be silent.
...
`M8-14`'s calendar half ("before the counted league") cannot be verified before a counted
...
from tests.conformance.neutral_stub import TOOLS
...
def test_the_four_tools_are_the_option_b_set() -> None:
    ...
    assert set(current_surface()) == {"negotiate", "receive_turn", "submit_audit",
                                      "receive_control"}
    assert "receive_move" not in current_surface(), "excluded by the Option-B decision"
