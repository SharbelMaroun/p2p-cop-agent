"""Is this repository submittable, by the book's own two lists (`M9-14`, `M9-02`)?

Both lists are quoted, not inferred, and both carry a stated penalty.

**9.4.1, Mandatory Repository Contents** (`inst/police_thief_p2p_Summary.md:2267`): a
README.md holding the academic report, configuration files, a PLAN, and a TODO — because
they "tell the development story and allow the examiner to reconstruct the work process, not
just the final result".

**9.4.2, README.md contents** (`:2283`): six components, where "the absence of any of these
will result in a grade deduction".

Presence and shape only, never quality. Whether this repository's Dec-POMDP section is any
good is a human judgement; whether it still exists after a refactor is not, and that is the
question worth asking automatically the night before a deadline. A checker that graded prose
would return a confident wrong answer about the thing that actually costs marks.

Two notes specific to this repository:

* **The companion link points at the Thief.** Rule 49 wants four repository URLs across both
  groups, and 9.4.2 item 6 wants this README to reach the other side of the pair. The
  pattern here is `p2p-thief-agent`; the Thief's copy of this checker looks for
  `p2p-cop-agent`, which is the one asymmetry between them.
* **`shared_contract/` is checked too.** It is not in the book's list — it is this
  repository's own obligation under `G-18`, since the Cop owns the shared wire bundle and a
  submission without it leaves the Thief's conformance claims unverifiable.

Written against this repository's own layout. The Thief solved the same problem the same
day; the reasoning travelled and the file did not (`M1-015`).
"""
...
MANDATORY_CONTENTS = (
    ...
    # Not in the book's list. This repository owns the shared wire bundle under `G-18`, and
    # submitting without it leaves the Thief's conformance claims unverifiable by anyone.
    ("shared_contract — the wire bundle this repository owns", ("shared_contract",)),
)
...
README_COMPONENTS = (
    ...
    ("6. Link to the companion repository", r"github\.com/\S+/p2p-thief-agent"),
)
