"""Hold the planning documents to each other (`G-10`..`G-14`).
...
* `G-10` every document in `docs/` appears in `DOCS_COMPLETENESS.md` with a status
* `G-11` a milestone may not read `DONE` in `PLAN.md` while `TODO.md` still has open rows
  for it, or the reverse
* `G-12` a requirements row naming a test names one that exists
* `G-13` every `U-nnn` cited in `TODO.md` is registered, and every registered one says what
  it blocks
* `G-14` every ADR carries a status, and a superseded decision says so rather than being
  quietly rewritten
...
HISTORICAL = ("SESSION_HANDOFF", "BLOCKER_RESOLUTION_AUDIT", "LECTURER_DIRECTION")
...
def check_docs_completeness() -> list[str]:
    """`G-10`. Every document present is listed, and every listed one has a status."""
    ...
    for path in sorted(DOCS.glob("*.md")):
        ...
        if path.name not in listed:
            findings.append(f"G-10: docs/{path.name} exists but DOCS_COMPLETENESS.md "
                            "does not list it")
    for row in re.findall(r"^\| `([^`]+)` \| *([^|]*)\| *([^|]*)\|", listed, flags=re.M):
        name, present, status = (cell.strip() for cell in row)
        if not status:
            findings.append(f"G-10: {name} is listed with no content status")
        if present.lower() == "yes" and not (DOCS.parent / name).exists():
            findings.append(f"G-10: {name} is listed as present but is not in the tree")
    return findings


def milestone_states() -> dict[str, str]:
    """Milestone → state as `PLAN.md` reports it."""
    rows = re.findall(r"^\| (M[0-9.]+)[^|]*\| *([A-Z ]+?) *\|", read("PLAN.md"), flags=re.M)
...
def check_plan_matches_todo() -> list[str]:
    """`G-11`. The two files disagree in both directions, and both matter.
    ...
    for milestone, state in milestone_states().items():
        ...
        if state == "DONE" and milestone in open_now:
            findings.append(f"G-11: PLAN.md calls {milestone} DONE, TODO.md still has open rows")
        if state in OPEN_STATES and milestone not in open_now:
            findings.append(f"G-11: PLAN.md calls {milestone} {state}, but every TODO.md row "
                            "for it is closed")
    return findings


def check_task_ids_are_unique() -> list[str]:
    """`G-11`. Two rows sharing an ID make the ledger unciteable.
    ...
    for task, state in re.findall(r"^\| ([MGX][\w.-]+) \|[^|]*\| *([^|]*?) *\|",
        ...
        if task in seen:
            findings.append(f"G-11: task id {task} is used twice "
...
def check_requirements_have_tests() -> list[str]:
    """`G-12`. A row pointing at a test file must point at one that is there."""
    ...
    for name in re.findall(r"`(test_[\w./-]+\.py)`", read("REQUIREMENTS_LEDGER.md")):
        if not list(root.rglob(Path(name).name)):
            findings.append(f"G-12: REQUIREMENTS_LEDGER.md cites {name}, which does not exist")
    return findings


def check_unknowns_are_registered() -> list[str]:
    """`G-13`. Both directions: no dangling citation, no unknown that blocks nothing."""
    ...
    for cited in sorted(set(re.findall(r"\b(U-\d{3})\b", todo)) - registered):
        findings.append(f"G-13: TODO.md cites {cited}, which UNKNOWN_REQUIREMENTS.md "
                        "does not register")
    for row in re.findall(r"^\| (U-\d{3}) \|[^|]*\| *([^|]*)\|", unknowns, flags=re.M):
        if not row[1].strip():
            findings.append(f"G-13: {row[0]} does not say what it blocks")
    return findings


def check_adrs_have_status() -> list[str]:
    """`G-14`. A decision that changed is marked superseded, never silently edited."""
    findings = []
    for adr in sorted((DOCS / "adr").glob("ADR-*.md")):
        text = adr.read_text(encoding="utf-8")
        if not re.search(r"^\s*(?:[-*>#\s]*)?\**Status\**\s*[:|]", text, flags=re.M | re.I):
            findings.append(f"G-14: {adr.name} carries no Status line")
        elif re.search(r"supersed", text, flags=re.I) and not re.search(
                r"Status\**\s*[:|]\s*\**\s*(?:Superseded|Accepted|Amended)", text, flags=re.I):
            findings.append(f"G-14: {adr.name} discusses supersession but its status does "
                            "not record one")
    return findings
