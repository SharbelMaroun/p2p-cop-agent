"""Phase-C barrier-containment matrix: contain_barrier vs denial (structural pursuit gap).
...
stream -- the larger evaluation, run only after PASS and never tuned against.
"""
...
from p2p_cop_agent.strategy.contain_barrier import contain_barrier_turn_intent  # noqa: E402
...
ARMS = {"denial": denial_turn_intent, "contain_barrier": contain_barrier_turn_intent}
...
def _starts(mode: str, count: int):
    ...
    pairs, rng = [], random.Random(0x9E3779B9)
...
def main() -> int:
    ...
    (RESULTS / f"contain_barrier_{mode}.json").write_text(
    ...
    print(f"{'thief':16s}" + "  ".join(f"{a:>16s}" for a in ARMS) + "   delta")
    for t in THIEVES:
        cells = "  ".join(f"{grid[a][t]['captures']:>4}/{grid[a][t]['games']:<10}" for a in ARMS)
        delta = grid["contain_barrier"][t]["captures"] - grid["denial"][t]["captures"]
