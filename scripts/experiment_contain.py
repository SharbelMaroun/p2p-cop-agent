"""Phase-C development + holdout matrix: containment vs denial (structural pursuit gap).
...
mode `holdout` sweeps `seeds` unseen random start pairs (default 60) with a disjoint seed
stream -- the larger evaluation, kept separate from the development gate.
"""
...
from p2p_cop_agent.strategy.contain import contain_turn_intent  # noqa: E402
...
ARMS = {"denial": denial_turn_intent, "contain": contain_turn_intent}
...
def _starts(mode: str, count: int):
    ...
    # Disjoint seed stream from the Phase-B holdout (0xC0FFEE): these starts are unseen.
    pairs, rng = [], random.Random(0x40D0)
...
def main() -> int:
    ...
    (RESULTS / f"contain_{mode}.json").write_text(
    ...
    print(f"{'thief':16s}" + "  ".join(f"{a:>10s}" for a in ARMS) + "   delta")
    for t in THIEVES:
        cells = "  ".join(f"{grid[a][t]['captures']:>3}/{grid[a][t]['games']:<6}" for a in ARMS)
        delta = grid["contain"][t]["captures"] - grid["denial"][t]["captures"]
