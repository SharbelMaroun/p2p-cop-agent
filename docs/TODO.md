...
| M5-07c | Rehearse a game across two machines over the tunnel | BLOCKED — needs the `serve` CLI (one unknown) + hardware + M8 evidence | P0 | Book stage 5 milestone: a remote agent plays a full game. **Decided 2026-08-01, revised 2026-08-02 and 2026-08-03.** Blocker progress: (1) *the autonomous play loop* — **RESOLVED by `M5-17`**; (2) *the pre-play protocol sequence* — **RESOLVED by `M5-17f`**: `orchestration/match.py` `play_match` negotiates, exchanges & verifies Step-0, locks the declaration, and plays, all in-memory-proven; (3) *the team-identity config source* — **SETTLED 2026-08-03 (Amr, with book Appendix B.4 citations)**: identity lives in the private `game.toml` `[game]` (group_id/group_name/members/repos) and `[llm].model`, the MCP URL derives from `[network].public_url`, and the hardware `spec` is os/cpu auto-detected with ram/gpu/vram operator-declared in `[hardware]` (the book requires signing *true* specs). `shared/team_config.py` (`load_identity`, `load_host_spec`) reads it, 100% branch, and `config/game.toml.example` gained the `[hardware]` section; the network `serve` command that assembles `play_match` from this over a live socket is the last code item, runbook-only (no CI without a socket); (4) *hardware* — two physical machines and a live tunnel; (5) *evidence* — the milestone's mandated proof is "Live GUI (belief map) and Replay App (Verified OK) screenshots" (`police_thief_p2p_Summary.md:2295`), both **M8** deliverables, so `M5-07c` cannot be *evidenced* until M8 exists. **What IS proven:** the localhost two-process test (`M5-09`/`M5-10`) crosses a real socket into a separate interpreter, `test_localhost_sub_game` plays a whole sub-game, `M5-17` closes the drive loop, and `M5-17f` closes the pre-play sequence. **Runbook (once `serve` and a tunnel exist):** on machine A start a tunnel (`ngrok http <my_port>` / cloudflare), set `[network].public_url` to the printed https URL, cross-set the two `opponent_url`s, launch each peer, and confirm a six-sub-game series completes with both audits verifying |
