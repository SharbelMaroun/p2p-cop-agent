...
| X-09 | Require the settlement record before a report can be composed | IN PROGRESS | P0 | **Found 2026-08-07 in the Thief**, where it was a layering gap rather than a missing check. `orchestration/settlement` already refused to *agree* without a passed audit, but the report composer took a bare result mapping — so a caller could skip settlement entirely and email a number the opponent had never confirmed. Neither `reporting/send_report.py` nor `reporting/gmail_message.py` mentions a settlement or an agreed state. Rule 36 makes the mutual audit a mandatory condition before agreement; rule 35 scores a conflicting report 0 for **both** teams, so an unaudited send maximises the expensive failure. Check `is not True`, not truthiness — a JSON round trip turning the flag into the string `"True"` passes a truthiness check `[AE-35]` `[AE-36]` |
| X-10 | Commit every game's config where the repository can carry it | IN PROGRESS | P1 | **Found 2026-08-07 in the Thief**; `M7-27` is the Cop's own deferred row for the same obligation. `.gitignore` excludes `/logs/`, `/reports/generated/` and `/results/generated/` — a sound default for run output and exactly wrong for the one artifact Appendix F obligation 4 says to commit. The failure is silent: the write succeeds and the file is there. The Thief added a `games/` directory that is deliberately not ignored, a store that **refuses** a destination under an ignored path, and a test that reads the real `.gitignore` and fails if `games/` is ever added to it — the way this regresses is somebody tidying the working tree. Match the path components, not a string prefix: `results/generated/` is ignored while a bare `results/` is not |
...
| M7-09 | Declare games already played against each opponent | IN PROGRESS | P0 | Appendix E rules 37/38: every game start carries an accurate count of prior counted games against that opponent, derived from emitted result artifacts rather than hand-entered. A false declaration is absolute disqualification, so the count is reproducible from the artifact set |
| M7-09a | Derive the count from emitted result artifacts | IN PROGRESS | P0 | No hand-entered figure can enter the declaration |
| M7-09b | Exclude warm-up games from the counted total | IN PROGRESS | P1 | `[AE-52]`; warm-ups are permitted but uncounted |
| M7-10 | Attach every game's configuration artifact to the repository | IN PROGRESS | P1 | Appendix F.2 items 3 and 4: each game's configuration artifact is named from its `game_id` and committed to the repository, so any past game's exact configuration remains retrievable |
| M7-10a | Commit each game's config under a `game_id`-derived name | IN PROGRESS | P1 | Artifacts from different games cannot collide |
| M7-10b | Prove any past game's config is retrievable from the repo | IN PROGRESS | P1 | A retrieval test walks the committed set |
| M7-11 | Account for LLM tokens across a series | IN PROGRESS | P1 | Per-game and per-series totals are counted, sealed at Step-0, and reported `[AE-54]` |
| M7-12 | Emit warm-up games as uncounted | IN PROGRESS | P1 | A warm-up produces artifacts but never enters the counted total `[AE-52]` |
...
| M7-15b | Refresh the access token automatically | IN PROGRESS | P1 | The refresh token gives months of autonomy |
...
| M7-15d | Document the five setup steps for a fresh machine | IN PROGRESS | P2 | Reproducible by a teammate `[G§2.1]` |
...
| M7-19 | Implement series-level score aggregation evidence | IN PROGRESS | P1 | The cumulative figure is reproducible from the artifact set |
| M7-19a | Recompute the series total from stored artifacts | IN PROGRESS | P1 | No in-memory-only total is trusted |
| M7-19b | Apply the diversity reward for a new opponent | IN PROGRESS | P1 | `[AF-t18]`; a repeat opponent adds nothing |
...
| M7-21 | Document the reporting pipeline | IN PROGRESS | P2 | `PRD_gatekeeper_reporting.md` matches the built gates and flow |
...
| M7-27 | Verify every emitted artifact is committed | IN PROGRESS | P1 | Appendix F.2 item 4; nothing exists only on a local disk |
