...
| M5-17f | Sequence negotiation and first move autonomously | PENDING | P0 | **Not done, and deliberately not stubbed.** Hosting and readiness are now done (`M5-17e`); what remains is the *protocol* half. Confirmed 2026-08-02: the reference's runtime **waits for the opponent's counter-signature before step 1** — "play starts only after both verifications pass" — so this needs send-offer → poll the agreements inbox → verify both directions → begin play. The book adds two requirements the reference does not settle: the Step-0 attestation must be **exchanged and mutually signed**, and the pre-game declaration (`_schema`, `schema_version`, `declaration_type`, `game_id`, `game_uid`, `links`, `timezone`, `game_started_at`, `game_ended_at`, `num_sub_games`, `max_tokens_per_game`, `groups`) must be written **after negotiation but before play** — "sign it and lock it cryptographically before play". A **passive** `serve` that comes up and mailboxes without playing was rejected on 2026-08-01 as proving connectivity rather than a game; that decision stands, which is why no `serve` command is wired until this row closes |
