...
| M5-07c | Rehearse a game across two machines over the tunnel | BLOCKED — needs hardware + the over-wire play loop | P0 | Book stage 5 milestone: a remote agent plays a full game. **Decided 2026-08-01: genuinely not closeable in this environment, and not faked.** Two real blockers: (1) *hardware* — two physical machines and a live tunnel, which cannot be unit-tested; (2) *the autonomous over-wire play loop* — `adapters.build_server` is a passive mailbox (enqueue + ack), so a full game still needs the drain wired to the orchestrator driving turns against a live peer's responses. That loop is the runtime gap the original handoff flagged and is the M5→M6 bridge; a passive-mailbox `serve` alone would prove connectivity, not a game, so it was deliberately not stubbed. **What IS proven:** the localhost two-process test (`M5-09`/`M5-10`) crosses a real socket into a separate interpreter, and `test_localhost_sub_game` plays a whole sub-game — the same carrier-independent code path. **Runbook (once the loop and a tunnel exist):** on machine A start a tunnel (`ngrok http <my_port>` / cloudflare), set `[network].public_url` to the printed https URL, cross-set the two `opponent_url`s, launch each peer, and confirm a six-sub-game series completes with both audits verifying. Note: there is **no** `serve` CLI yet — `python -m p2p_cop_agent` is still the scaffold, so launching a peer is itself part of the remaining work. Awaiting the play loop, then Amr's cross-machine run |
