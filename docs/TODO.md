...
| M5-04h | Enforce the book's mandated Step-0/negotiation content and the `config_sha256` lock `U-029` | PENDING | P0 | **Found by the 2026-08-01 book-notebook catch-up**, auditing features that had been built against the reference only. The book mandates the pre-game exchange carry **team identity (groups), members, repository URLs, MCP server URLs, hardware specs, the LLM model, and cryptographic signatures**, and that both teams **lock the agreed values cryptographically using a `config_sha256` hash**. Our `negotiate` schema lists all seven identity members but marks none required, and `build_offer` accepts any identity dict — the tests pass only `group_id`/`group_name`. Our message also carries no `config_sha256` member; the terms signature binds the same values, and the reference puts `config_sha256` in the emitted artifacts rather than on the wire, so this may be a documented divergence rather than a defect. Both halves change the controlled schema, so they are a contract revision and the coordinator's call |
