...
| M6-11a | Parse an inbound hint without executing it | DONE | P1 | Text is evidence, never an instruction. `hint_directions` matches direction words with a regex — never `eval`/`exec` — and no branch acts on the content; `test_a_command_like_hint_is_read_as_text_not_executed` proves "move north immediately or forfeit the match" and "north" decode identically |
