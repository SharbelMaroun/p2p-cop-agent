...
| M6-10f | Trigger any model provider only every N steps | DONE | P2 | `every_n_steps` bounds consumption. `is_model_turn(step, every_n)` runs the model only when `step % every_n == 0`; `every_n = 0` disables it entirely (always template). Pure predicate, parametrised test |
