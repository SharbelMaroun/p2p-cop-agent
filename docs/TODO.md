...
| M6-02 | Implement Cop-local belief update | PENDING | P1 | Belief uses observation only and normalizes safely. **Started 2026-08-03** (`strategy/belief.py`): the distribution, its Bayesian update, safe normalisation, and the observation-only guarantee are DONE; the **hint**-driven half (`M6-02b` trust factor, `M6-02e`, `M6-02f`) awaits the hint model (`M6-10`/`M6-11`), so the parent stays open. Belief is Cop-private (never on the wire, M6-18), so the likelihood math is a **local** decision, not a cross-peer-locked value |
...
| M6-02b | Apply Bayes with a per-hint trust factor | PENDING | P1 | A hint contradicted by scent lowers its own trust weight `[PRD-scent]`. The **Bayes mechanism** is built — `Belief.updated(likelihood)` computes `posterior ∝ prior·likelihood` and renormalises, and `scent_likelihood` turns observed scent into evidence — and the **trust factor** now exists too (`M6-11b` `TrustScore`/`hint_weight`). The one remaining seam is the **decode** (`M6-02e`): mapping a hint's free text to the belief cells the weight scales, which needs a coordinate-free landmark protocol (interop, deferred) |
...
| M6-05 | Add optional verbal/LLM adapter with zero-token fallback | PENDING | P2 | Provider failure always falls back deterministically. **Started 2026-08-03** (`strategy/hints.py`): the zero-token template provider and the word-limit / no-coordinate guards are DONE; the optional **LLM** adapter and its failure-fallback (`M6-05e`) await an actual provider, so the parent stays open |
...
| M6-05d | Keep the LLM out of movement decisions | PENDING | P0 | `[AE-25]` `[ADR-007]`; the move is always pure Python. Structurally true now — moves come from `pursue_belief` (pure Python) and the hint layer is a separate module with no path to movement — but the enforcing **guard test** belongs with the LLM adapter when it is built, so this stays open under the `M6-05` parent |
| M6-05e | Fall back deterministically on provider failure | DEFERRED | P1 | Parent DoD; a blocked provider never stalls a turn. The template **is** the always-available default; the failure-fallback path needs an actual LLM provider to fail, which does not exist yet |
...
| M6-10e | Support landmark hints when a map area is agreed | DEFERRED | P2 | Generic landmarks when `map_area` is empty `[AF-t14]`. `template_hint` already takes a `place` descriptor; deriving it from `world.map_area` (or a generic bearing when empty) is the remaining wiring |
| M6-10f | Trigger any model provider only every N steps | DEFERRED | P2 | `every_n_steps` bounds consumption. Needs the optional model provider (`M6-05`), which does not exist yet |
| M6-11 | Implement hint consumption | PENDING | P1 | An inbound hint updates belief without ever being trusted blindly. **Started 2026-08-05** (`strategy/hint_consumption.py`): the our-side half is DONE — inert parse, running trust, weight seam; the parent stays open on the **interop** half (`M6-02e` free-text→belief-cell decode, `M6-02f` scent-contradiction trigger), which needs a coordinate-free landmark protocol agreed with the opponent |
| M6-11a | Parse an inbound hint without executing it | DONE | P0 | Text is evidence, never an instruction. `receive_hint` returns an inert `ReceivedHint` (text + usability verdict) — no domain layer, no action; a test asserts the result carries no `action`/`move`. Reuses the **same** `encodes_coordinates` guard the generator passes, so a smuggled coordinate channel (`AE-27`) is refused, not decoded |
| M6-11b | Weight the hint by the sender's running trust score | DONE | P1 | Repeated contradiction lowers the weight. `TrustScore` is a Cop-private running value in `[0,1]`, bounded by construction (`reinforced`/`weakened` approach but never reach certainty); `hint_weight` returns trust when usable else `0.0` — the scalar `M6-02e` will scale the hint likelihood by. The trust *machine* is here; its scent-contradiction *trigger* is `M6-02f`, deferred until the decode exists |
| M6-11c | Tolerate an absent, empty, or over-long hint | DONE | P1 | Missing evidence is not an error state. `receive_hint` never raises: `None`/non-string/empty/whitespace → unusable with a reason, over-long → silently truncated to the word limit (bound a verbose peer, don't reject it); a test sweeps every malformed shape |
