...
| M5-17e | Launch a peer as a long-running process (`serve` CLI) | PENDING | P0 | **Not done, and deliberately not stubbed.** `python -m p2p_cop_agent` is still the scaffold. Two parts remain: (1) running `build_server(...).run(transport="http", …)` — a *blocking* call — in a background thread alongside the poll loop; (2) sequencing the negotiation handshake over the wire autonomously, including the case where the opponent's tunnel is not up yet. A **passive** `serve` that only mailboxes was rejected on 2026-08-01 as proving connectivity rather than a game, and that decision stands. Now unblocked by `M5-17`: the loop such a CLI would drive exists and is proven. **Bind address, confirmed in the book 2026-08-02 (`police_thief_p2p_Summary.md:657`): `mcp.run(transport="http", host="0.0.0.0", port=…)`, with the book's own comment "Bind the server so a tunnel can expose it publicly".** This is a trap, not a detail: `tests/integration/localhost_peer.py` binds `127.0.0.1`, and a `serve` copying that harness would start cleanly, pass every local check, and be **unreachable through the tunnel** — failing only at the stage-5 rehearsal, where it looks like a network problem rather than a one-word bug |
