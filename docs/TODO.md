...
| X-11 | Give the secret scanner its own tests | OPEN | P1 | **Found 2026-08-07 in the Thief.** Its scanner's detection rules were unexercised because `findings()` read a file and formatted a repository-relative path in one pass, so it could only be tested through a file *inside* the repository. Splitting a `line_findings(line)` out fixed that. Worth checking the Cop's scanner for the same shape: this gate has caught two real problems in the Thief, and a scanner quietly widened is worse than no scanner because the repository goes on passing. When the scanner flags its own test file — it will — rebuild the probes as runtime-joined fragments rather than allowlisting. An exemption for the one file where a real key would look at home is the worst one available `[AE-39]` `[AE-40]` |
