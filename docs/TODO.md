...
| M3-03c | Encode the technical-loss zero | DONE | P1 | `[AE-19]` `[AE-48]`; counterpart award stays open as `U-026` |
...
| M3-07 | Resolve the step-limit / survival-threshold boundary | PENDING | P0 | Appendix F table 15 sets `[Step Limit]` and `[Survival Threshold]` to the same value (35), leaving it undefined whether surviving *exactly* the threshold is a Thief win. `run_sub_game` already resolves it as a win without recording the choice. Owner ruling recorded as `U-027` with a boundary test either way |
...
| M5-04c | Validate participant identity and ordering | DEFERRED | P0 | `agreed_between` ordering is canonical and stable |
| M5-04d | Refuse below-minimum and altered fixed values | DEFERRED | P0 | `[AE-12]` `[AF-§1]` |
| M5-04e | Prove propose and accept directions both pass | DEFERRED | P0 | Neither direction requires editing a profile file |
...
| M5-11d | Emit a structured log line per phase transition | DEFERRED | P1 | The log manager receives every transition |
...
| `U-026` | What the non-falsifying peer scores when its opponent takes a technical loss. The book fixes 0 for the forger and is silent on the counterpart | `M3-03c`, `M7-06` |
