...
| X-06 | Align the artifacts to the lecturer's templates | IN PROGRESS | P0 | **Claimed 2026-08-06.** Found by reading a notebook answer as *text* rather than from a screenshot -- the code notebook had already given the roster and it went unregistered. Our config artifact omits `agreed_between` and `config_name` and calls the sub-game key `sub_game` where the templates and `inst/:3019` say `sub_game_number`; our log splits reveals into `steps` + a separate `audit` section where the templates use one `records[]` with the nonce added inline at the end; and our declaration's `links` holds repo URLs where the template holds the four artifact filenames (rule 49's four repo links are a *separate* requirement I collapsed into it). **The Thief already matches the templates on all three**, so the two repos currently emit different shapes for the same game. `per-subgame-config.schema.json` requires `sub_game`, so this needs a bundle bump `[G-18]` |
