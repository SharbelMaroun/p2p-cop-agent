...
| M5-17f | Sequence negotiation and first move autonomously | PENDING | P0 | **Decomposed 2026-08-02; two of three children DONE, the third M7-gated.** Both protocol halves the reference and book require before the first move are built: the agreement gate — "play starts only after both verifications pass" (`M5-17f-i`) — and the mutually-verified Step-0 attestation exchange (`M5-17f-ii`). The one remaining child, `M5-17f-iii` (the pre-game declaration written and cryptographically locked before play), is **owned by M7** (`M7-02a`/`M7-22`): its artifact and schema do not exist yet, so it is `DEFERRED`, not open work under this row. Parent therefore stays `PENDING` — a milestone rule (all children `DONE`), not remaining protocol design — and closes when `M7-02a` provides the declaration builder and the `serve` preamble wires it in before play. A **passive** `serve` that comes up and mailboxes without playing was rejected 2026-08-01 and again 2026-08-02; that stands, so no `serve` command is wired until the declaration lock closes the preamble |
