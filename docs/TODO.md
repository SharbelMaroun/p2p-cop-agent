...
| M7-28 | Emit the group name alongside the group id in the declaration | OPEN | P1 | **Found 2026-08-07 while closing `M7-02`.** The book requires group identity as *name, group_id and members* (p.39/104, 78/183); `protocol/declaration._group` emits `group_id`, `members`, `repos`, `mcp_servers`, `llm_model` and `spec` but no `group_name`. The declaration schema validates it when present and does **not** require it, because a schema that rejects the only artifact this repository can produce is `X-04` repeating. Promote to required in `shared_contract/schemas/declaration.schema.json` once the builder supplies it. The companion Thief already carries `group_name` `[AE-24]` |
