...
**2026-08-09 first live opponent.** Two rows closed on real evidence. `M8-03c` — a
friendly, uncounted sub-game was played in the Police role against group `amireman`
across two machines, and the three defects it exposed are recorded there. `M9-05a` — the
annotated `v1.0-submission` tag is pushed. **M9 itself has not started**: every counted
league row, the final audit, and the whole Moodle path remain `DEFERRED`, and a friendly
game deliberately counts for none of them `[AE-52]`. The remaining M9 work is
user-side — playing counted opponents, arming OAuth reporting, and submitting — so it
...
passes: `the Cop` reproduced **17 of 17** CORE vectors -- canonical JSON (Hebrew, astral
...
omission: `U-028` already settled the key as tolerated-when-present, the terms projection
...
`max(0, (1-rho)tau + delta)` with no upper clamp, because `U-031` is still open and the book
never settles it. The emission kernel itself is *identical* to their `BOOK_KERNEL` across all
...
shared match object may carry `pheromone_min_center_intensity`, and that is `U-028`, already
...
Both contradict the settled rule -- `C-031`/`U-029` here, `U-024` there: *populate ours,
tolerate theirs*, with cross-peer enforcement explicitly open. Rule 24's sanction is the loss
...
*content* and its sanction is a bonus denial, which is what settles this, and `U-024`/`U-029`
already record that refusal is undecided. No notebook could have said more than the open
...
**2026-08-16 the wire shape, and two harnesses that were never measuring us
(`C-052`).** `yanell11` said our `smell_grid` should be the accumulated trail, not a 5x5
window, and pointed at the reference source instead of arguing it. Verified there and they are
right: the field persists across turns, `snapshot()` returns the whole thing, and
`smell_grid_size` sizes the emission **kernel**, not the transmitted dict. Our window was the
deviation -- from the Option-B profile this repository adopted -- and it was also *harder for
an opponent to invert than the shape it was imitating*.

Fixed: `ScentField.snapshot()`, transmitted by `live_policy`; `window()` stays as a local view.

**Then the interesting part.** Chasing the eight capture-rate tests that `C-048`'s clamp had
turned red found that neither harness was measuring this agent:

* every arena script fed `field.window(thief)` -- so **every published capture-rate number was
  taken against a shape no peer sends**. A 25-cell window loses half its cells to the clamp's
  saturation filter; a 46-cell trail does not. Measured against a trail generated by the kit's
  own `ref_book_full_turn`: **16/16 turns localised from the trail, 12/16 from the window**.
* `compare_strategies._belief` used `scent_likelihood`, the raw-intensity weighting `M6-031`
  replaced, while the live path has used `emitter_likelihood` since `M11-01`.

Both corrected. **All eight failures cleared** -- six from the shape, two from the arm -- so
the clamp's measurable cost to the shipped agent is **zero**. That number was wrong all day
and it was wrong in our favour to believe it, which is why it stood unexamined: a plausible
cost for a fix we had already decided was mandatory.

**This is `M11-01`'s lesson repeating one layer up.** That entry replaced a decoder that was
"grading its own homework" with a geometry fix -- which reads a window shape **only our own
emitter produces**, and therefore fired 3 times in 102 observations against a real opponent. We
replaced one self-referential assumption with another and measured both against ourselves. The
rule worth keeping: a measurement whose inputs we generate cannot tell us whether we
interoperate, and the only cure found so far is to generate the inputs from *their* code.

**Method.** Steps 1, 2, 4, 5, 6, 7 ran. Step 3 could not (extension unavailable); the source
used instead was the reference implementation itself, which is what the code notebook exists to
answer questions about and outranks it.
...
| M5-14c | Wire the bounded re-send into the live turn delivery | DONE | P0 | **Closed 2026-08-09.** `M5-14a` proved `attempt(..., retry_on=(TransportError,))` retries a carrier fault and not a rejection — but **only in `test_deadlines`**: no production path ever called it, so `turn_loop._deliver` sent once and any `TransportError` went straight to `machine.fail()` and `sub_game` to `TECHNICAL_LOSS`, with Appendix F's three permitted retries unused. One dropped HTTPS request — the ordinary behaviour of the free tunnel both peers actually run on — therefore lost a sub-game outright, while the deadline rule the constants serve says *"retry **or** declare a technical loss"* and we only ever did the second half. `orchestration/delivery.py` (`DeliveryRetry`, `send_turn`) now wraps the send, `match._play` builds the policy from the **signed** match object so neither peer can grant itself a longer rope, and `run_turn`/`run_sub_game_over_wire` thread it through with a `None` default that leaves every existing caller single-attempt. **Only `TransportError` is retried**; `PeerRejectionError` (a decided outcome) and `CommitRevealError` (our own sealing defect) propagate on the first raise — the distinction `_deliver` had documented since M5-14a but never exercised, because nothing retried. The turn is re-sent, never re-sealed, so the commitment hash the audit checks is untouched (M5-11b). Bounded by construction: `attempt` opens a fresh `response_timeout_sec` deadline per try and retries only while it is unexpired, so a **fast** failure costs the backoff (4 attempts + 3×5 s ≈ 15 s, inside the 60 s watchdog) and a **slow** one — a request that hangs to its own expiry — is not retried at all, which is what keeps the opponent's watchdog out of it. Accepted cost, deliberately: duplicate delivery becomes ordinary traffic, safe against our idempotent inbox (`M5-05e`) but a peer that reads a repeated step as a replay attack would see one — worth telling an opponent before a counted game. `test_delivery.py` (7) |
...
| M6-08b | Parse an opponent field defensively | DONE | P0 | Non-numeric, NaN/infinity, boolean, negative and above-saturation intensities, non-string keys, and keys that are not an integer pair all reject; a corrupt grid **raises** rather than degrading to empty, because scent is the one channel that cannot be faked. **Corrected 2026-08-09: an off-board cell is now dropped, not fatal** — the defect that lost the `amireman` friendly. The reference transmits a *fixed-size* 5×5 window and keeps its zeros, so a peer anywhere but the interior necessarily names cells that do not exist (negative ones near the origin, past-the-edge ones near the far corner); at the 7×7 floor only the central 3×3, **9 of 49 cells**, has a fully on-board window. Refusing the whole grid for them blinded the Cop across the other **82%** of the board — and a *fleeing* Thief is in that 82% by definition — after which `M6-27`'s flat belief parked it on its start cell. Verified end-to-end: a foreign fixed-size encoder walking to the corner is now tracked **7/7** where the pre-fix parser decoded 0 cells from turn 3 on. The key regex admits a leading `-` so a negative index reads as *elsewhere* rather than *malformed*; the module's own "generous in what we accept" rule had been implemented for absent cells but not extra ones |
...
| M6-27 | Sweep when the belief localises nothing, instead of standing on the start cell | DONE | P0 | **Closed 2026-08-09, from the `amireman` friendly log.** A flat belief's row-major argmax is `(0,0)` — the Cop's own opening square — so the stack read "already on target" and served `STAY`: in the played game **26 consecutive stays, 0 barriers, 5 of 49 cells visited, lost on the horizon**. Three paths reach a flat belief and each froze identically: never observed, a decode that yielded no cells, and an observation whose likelihood carried no evidence (`_observe` rebuilds from `Belief.uniform(...)` every turn, so `updated`'s zero-evidence guard *returns the uniform prior* rather than keeping the old peak). A fourth froze on a stale peak: observations stop, the Cop reaches the last known cell and sits there. `strategy/patrol.py` answers all four with one predicate — `needs_sweep` (never seen, stale beyond 3 turns, or no peak above `PEAK_MARGIN`) — and `aim` returns a deterministic waypoint tour instead, skipping a waypoint the Cop already occupies (otherwise "go where you are" is `STAY` again). Replaying the friendly's message sequence: **1 → 15 distinct cells; the never-observed case 1 → 22**. Moving cannot improve what we *see* (the window is the opponent's to send) but occupying the Thief's cell is itself the capture. `test_patrol.py` (10) + 2 in `test_live_policy.py`; the `M6-25` tournament bounds are unchanged |
...
| M8-03c | Rehearse against a real classmate agent before the counted league | DONE | P0 | **Played 2026-08-09 against group `amireman`, friendly and uncounted `[AE-52]`.** Sub-game 1 in the Police role over their Cloudflare tunnel: `match outcome: SURVIVAL after 35 step(s)`. It was a live opponent and not self-play — `declaration_game-772de8f029e4.json` carries both groups with `amireman`'s own repositories and endpoint — and `replay` re-verifies the log, "Verified OK", 34 steps re-verified, sequence intact 1..34. Artifacts in `games/friendly-amireman/`. **The warm-up paid for itself**: three defects had passed every local gate and surfaced only against a real peer — the readiness probe read a CDN `502` as a live origin (`M9-26`), the match-config schema rejected `pheromone_min_center_intensity` and so refused every simulator-shaped opponent, and the declaration builder aborted an already-agreed match when the opponent withheld `mcp_servers`. Each is correct on localhost and wrong across a tunnel, which is precisely what a rehearsal exists to find. Sub-games 2–6 of the series are not yet played |
...
| M9-05a | Push an annotated `v1.0-submission` tag | DONE | P0 | Tag object `a3cacf0` — a real annotated tag, not lightweight — points at `961fc81` and is present on `origin`. **Re-point it if the submission commit moves**: it currently equals `origin/main`, so any further commit leaves the tag behind, and rule 41 wants the tag on the commit actually submitted `[AE-41]` |
...
| M9-27 | Emit a real ISO-8601 turn `timestamp`, and cap every outbound call | DONE | P0 | **Closed 2026-08-10, both defects found by reading an opponent's interop spec against our code.** (1) *Timestamp.* This peer sent `f"t{count}"` -- `"t1"`, `"t2"` -- an opaque counter. It satisfied `turn-message.schema.json` (`type: string`, `minLength: 1`) and so passed **every gate for the whole project**; the schema can check that a field is a non-empty string but not that it is a *time*. **Both sources were asked before changing it** (`M9-26` habit): the reference builds this field in `src/police_thief/peer/sealing.py` as ISO-8601 carrying a UTC offset, and the book pins no regex in the chapter-8 text but makes every absolute time field in the mandatory artifact templates ISO-8601 **with a timezone**, with section 8.4.1 (p. 65/157) requiring a real clock precisely so a peer can detect a frozen opponent -- which a counter cannot. **The sources also corrected our reasoning**: `inst/DEV-SPEC.md:400` reads as though the deadline is computed *from* this stamp, but the reference tracks turn timeouts from its own `time.monotonic()` and never trusts a peer's value, because two peers share no clock. So the field is for the audit record and interop, and nothing may start timing off it. `now_iso()` added to `protocol/messages.py`, mirroring the companion Thief, which has emitted ISO-8601 from the start -- only this side drifted. (2) *Call cap.* `FastMCPClient` has always accepted a `timeout`, but `serve` never passed one, so `self._timeout is None` and every live call waited indefinitely. The breach is arithmetic, not networking: the MCP SDK's per-call default equals the 30s we sign, so one delivered-but-unanswered push, one backoff and a second push exceed the deadline **while each call looks healthy** -- we breach a deadline we signed and hand ourselves the technical loss. `RetryPolicy.call_timeout_sec` derives the cap as the signed budget over `attempts` (7.5s of 30s at Appendix F's three retries; 10s at two, which is where the wider league independently landed). **Our own parametrised test found the hole**: at zero retries `attempts` is 1 and the division returns the deadline itself, so a `MARGIN` clamp keeps the cap strictly under for *every* retry count a peer can negotiate. 4 + 13 tests in `test_turn_timestamp.py` and `test_call_timeout.py`; `test_the_old_counter_shape_would_fail_this_gate` pins the regression itself |
...
| `shared_contract/` | The controlled bundle of schemas, fixtures, and golden vectors that both peers must satisfy. Currently `0.2.11-proposed`, `UNFROZEN` |
