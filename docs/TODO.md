...
| M7-02a | Emit `declaration_<game_id>.json` | IN PROGRESS | P1 | The **naming and atomic-write layer exists** (`reporting/naming`, `reporting/emit`) and `build_declaration` is complete (`M7-22a`–`M7-22e`). Open on the call: nothing in `orchestration/match` writes the declaration yet. Groups, members, both repos, MCP addresses, hardware, model, token budget, times |
| M7-02b | Emit `config_<game_id>_g<NN>.json` | IN PROGRESS | P1 | `build_config` is complete and schema-valid. Open on the same call site as `M7-02a` — the builders exist, the wiring into a played match does not. All quantitative parameters plus crypto locks and identity |
...
| M7-03a | Emit `log_<game_id>_g<NN>.json` with full commit-reveal | IN PROGRESS | P1 | Every step records hash, nonce, move, and hint — enough to re-audit |
...
| M7-24 | Make the log artifact sufficient for an independent audit | IN PROGRESS | P0 | A third party can re-verify without our code |
| M7-24a | Record each step's commitment and revealed payload | IN PROGRESS | P0 | Enough to recompute every hash |
| M7-24b | Record nonces only in the final audit section | IN PROGRESS | P0 | Nonce secrecy holds until the end `[AE-18]` |
| M7-24c | Record the hint and intent per step | IN PROGRESS | P1 | The verbal layer is auditable too |
