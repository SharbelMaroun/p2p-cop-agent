...
| X-07 | Carry rule 53's commit hash in the declaration | OPEN | P0 | **Found 2026-08-07 while finishing the Thief's M7**, where the same field was missing. Rule 53 (Mandatory) requires the commit hash of the code that played to be recorded in the declaration, and `github_commit` appears **nowhere** in `src/p2p_cop_agent/`. The declaration names who played, on what hardware and with which model, but never *which code* — the one field that makes a later audit reproducible. The Thief fixed this by adding a mandatory `github_commit` to `build_declaration` and refusing a hash shorter than 7 characters: a value ambiguous across the repository satisfies the rule's letter while defeating it. Mirror by re-authoring, not copying `[AE-53]` |
...
| X-13 | Correct the nonce reasoning wherever it was used to justify not committing logs | OPEN | P1 | **Found 2026-08-07 by asking the book notebook.** The Thief's `games/README.md` justified excluding game logs from the repository on the grounds that committing them would publish nonces. **That reasoning is wrong.** Rule 18 (`inst/:3354`) keeps a nonce secret *until the end of the game*, and the book defines Step 4 as the Final Reveal -- "Only at the end of the game are all values, including the Nonce, revealed for a full mutual audit" (`inst/:1136`, `:1155`). The obligation **expires**; revealed nonces are exactly what lets a third party recompute every commitment. Corrected in the Thief. Check whether this repository states the same wrong reason anywhere `[AE-18]` |
| X-14 | Narrow M9-09/M9-27 to the obligations the book actually states | OPEN | P1 | The book's commit obligations **differ per artifact** and treating them alike gets one wrong. Config: **mandatory** (Appendix F obligation 4, p.140/288). Game log: no explicit commit duty in 9.4.1's minimum contents, but needed to run the Replay app, which rule 20 makes a threshold condition (p.129/272). Final result: no commit duty -- its obligation is email (rule 51, p.133/279). A checker demanding all four would fail a submission that satisfies the rules. Ask **Git**, not the filesystem: a file on disk and untracked is the exact state the obligation prevents and is invisible to `Path.exists()` |
| X-15 | Record a send receipt without overclaiming delivery | OPEN | P0 | **`M9-09c` says "record proof that each report was sent", and the obvious reading overclaims.** The book's decisive layer is receipt at the lecturer's address (p.78/183): "if a report is not received from one of the sides, that side will not be credited for the game" -- and a sender **cannot observe receipt**, only the recipient can. The Thief named its type `SendReceipt` rather than `ProofOfDelivery` and writes `evidences: API acceptance, not receipt by the lecturer` into every record. The reference stores nothing: its sender returns `{status, reason}` for a CLI line that never reaches the artifacts `[AE-32]` |
| X-16 | Add a clean-clone gate runner | OPEN | P0 | **The Thief's caught two failures on its first run**, with every gate green in the working tree. It clones `HEAD` locally, `uv sync --frozen`, and re-runs the gates in the clone. The value is that a clone holds only what was committed, so a gate script living untracked in a working tree cannot hide -- the symptom otherwise is "command not found" during a grader's run. Not a substitute for a second machine: a local clone shares the OS, Python build and uv cache `[G-001]` |
| X-17 | Re-verify a stored match read back off disk | OPEN | P0 | **Every replay test in both repositories builds records in memory.** `json.dumps`/`loads` is not identity and the commitment is over canonical bytes, so a verifier that only ever sees in-memory dicts can pass forever while every stored log fails -- and the first person to notice would be whoever opened the submission. The Thief now plays a series, writes JSON, reloads **by path alone** and re-verifies to `Verified OK`, then changes a byte in the file and confirms `TAMPERED`. Rule 20 makes a verifying replay a threshold condition and p.81/189 makes the screenshot "absolute mandatory" `[AE-20]` |
