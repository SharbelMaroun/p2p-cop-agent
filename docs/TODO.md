...
| M5-07c | Rehearse a game across two machines over the tunnel | BLOCKED — needs hardware + a `serve` CLI + M8 evidence | P0 | Book stage 5 milestone: a remote agent plays a full game. **Decided 2026-08-01, revised 2026-08-02.** The blocker list is now three, and one of the original two is resolved: (1) *the autonomous play loop* — **RESOLVED by `M5-17`**: the mailbox is now driven, and `test_autonomous_play.py` plays a whole sub-game with nothing fed in by hand; (2) *launching a peer* — still open as `M5-17e`, needing a threaded server plus autonomous negotiation; (3) *hardware* — two physical machines and a live tunnel, which cannot be unit-tested. **New, found 2026-08-02 from the book notebook and confirmed at `police_thief_p2p_Summary.md:2295`:** the milestone's mandated **evidence** is "Live GUI (belief map) and Replay App (Verified OK) screenshots", both of which are **M8** deliverables. So `M5-07c` cannot be *evidenced* until M8 exists even once the hardware and CLI are there — it was previously recorded as hardware-only, which understated it. **What IS proven:** the localhost two-process test (`M5-09`/`M5-10`) crosses a real socket into a separate interpreter, `test_localhost_sub_game` plays a whole sub-game, and `M5-17` closes the drive loop. **Runbook (once `M5-17e` and a tunnel exist):** on machine A start a tunnel (`ngrok http <my_port>` / cloudflare), set `[network].public_url` to the printed https URL, cross-set the two `opponent_url`s, launch each peer, and confirm a six-sub-game series completes with both audits verifying |
...
| M5-17f | Sequence negotiation and first move autonomously | PENDING | P0 | **Decomposed 2026-08-02; the first and central sub-task is DONE.** The reference's runtime **waits for the opponent's counter-signature before step 1** — "play starts only after both verifications pass" — and that gate, the heart of this row, is built (`M5-17f-i`). Two book requirements the reference does not settle remain open as `M5-17f-ii` (Step-0 attestation exchanged and mutually signed) and `M5-17f-iii` (the pre-game declaration written and cryptographically locked after negotiation but before play). A **passive** `serve` that comes up and mailboxes without playing was rejected on 2026-08-01 as proving connectivity rather than a game; that decision stands, which is why no `serve` command is wired until this parent closes. Parent stays `PENDING` until all three children are `DONE` |
...
| M5-17f-ii | Exchange and mutually sign the Step-0 attestation | PENDING | P0 | **Not started; needs a wire-shape decision, so flagged for Amr/coordinator.** The seal primitives exist (`sdk.seal_step_zero_attestation`, `protocol.verify_attestation`, M4-06), and identity already carries hardware/model on the offer. What is undecided is *what crosses the wire pre-game and when*: the simulator profile reveals commitments only at the end-game audit, so exchanging the Step-0 **commit** before the first move — and whether to **require** the opponent's — is a `U-029`-style "populate ours, tolerate theirs" call, not a silent choice. Likely carrier is the optional `receive_control` channel (`additionalProperties: true`). Do our side (send ours; tolerate omission) once the shape is confirmed |
| M5-17f-iii | Write and lock the pre-game declaration | DEFERRED | P0 | **Owned by M7, gated here.** The declaration artifact (`_schema`, `schema_version`, `declaration_type`, `game_id`, `game_uid`, `links`, `timezone`, `game_started_at`, `game_ended_at`, `num_sub_games`, `max_tokens_per_game`, `groups`) and its schema are `M7-02a`/`M7-22`; building them here would preempt M7 and there is no declaration schema in `shared_contract/` yet. This row records only the **timing/lock obligation** M5 imposes — the declaration must be written after negotiation and cryptographically locked *before play* — which the future `serve` preamble must honour once `M7-02a` provides the builder. Cross-reference: `M7-22` |
