...
| M5-07c | Rehearse a game across two machines over the tunnel | BLOCKED — needs hardware | P0 | Book stage 5 milestone: a remote agent plays a full game. This cannot be unit-tested; it needs two physical machines and a live tunnel. **Runbook:** on machine A, start a tunnel (`ngrok http <my_port>` or a cloudflare tunnel) and set `[network].public_url` to the printed https URL; put A's URL in B's `[network].opponent_url` and vice-versa; run each peer with `python -m p2p_cop_agent`; confirm a full six-sub-game series completes and both audits verify. The localhost two-process proof (`M5-09`/`M5-10`) already exercises the same code path over a real socket; 07c only swaps the carrier for a public tunnel. Awaiting Amr's cross-machine run |
