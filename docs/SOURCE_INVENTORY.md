...
| Four course-supplied official JSON examples | Directly inspected and hashed in `ARTIFACT_TEMPLATE_BASELINE.md`; official designation derives from the supplied course handoff | Exemplar 1.1 key sets and filename evidence; not complete formal schemas or independently publisher-signed artifacts |
| Lecturer simulator | Public upstream pinned to `960499fd5e8777b4929625f5d8fdcf2ab4677b54` | Learning/interoperability reference; behavior is not automatically mandatory |
...
| Cross-team ADR accepted by both peers | None accepted for the proposed bundle yet | May decide permitted design choices, never override higher authority |
...
addresses, shared JSON/private TOML split, and artifact filename patterns. Remaining
questions must be named narrowly rather than describing those sources as unavailable.
