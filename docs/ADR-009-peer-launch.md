# ADR-009 — Launching a peer: threading, bind address, and readiness

*Status:* accepted 2026-08-02 · *Supersedes:* nothing · *Relates to:* `M5-17e`, `M5-07c`

## Context

`M5-17` closed the play loop: the mailbox is now driven, and a sub-game plays with
nothing fed in by hand. What is still missing is the process that hosts it. There is
no way to *launch* a Cop peer — `python -m p2p_cop_agent` is a scaffold that prints
help.
...
counter-signature before step 1. That is a separate sub-task (`M5-17f`) and is not
