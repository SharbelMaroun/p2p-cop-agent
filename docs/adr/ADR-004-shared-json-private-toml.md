...
- Parity-control `config/game.json` and `config/rate_limits.json`.
...
- Exclude secrets, ports, provider/model choices, credentials, tunnels, nonces,
  emails used as credentials, and strategy tuning from the shared bundle.
