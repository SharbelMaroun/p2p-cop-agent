...
| OB-001 | The project selects Option B (simulator-v3 profile) as a documented academic-freedom interoperability choice where the book leaves wire details open | Coordinator project decision dated 2026-07-28; [OPTION_B_DECISION.md](OPTION_B_DECISION.md) | New `0.2.1-proposed` bundle; conformance suite |
...
| OB-004 | Move/negotiation commit is `sha256(canonical_json(payload) + "\|" + nonce)` with a literal `\|` delimiter; nonce is 16 random bytes as 32 lowercase hex, outside the payload, revealed only in the post-game audit | ADR-006 move-commit accepted under Option B | Canonicalization vectors (WP5) |
...
`0.2.1-proposed`, **UNFROZEN**, built as the role-neutral top-level `shared_contract/`
subtree. `0.2.1-proposed` corrects the barrier-placement rule and removes the
unauthenticated role-alternation schedule from the bundle; it supersedes
`0.2.0-proposed`, whose manifest hash refers to the earlier byte set. Remaining formal artifact schema constraints stay in
