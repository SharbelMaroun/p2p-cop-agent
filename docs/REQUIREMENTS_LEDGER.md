...
| OB-001 | The project selects Option B (simulator-v3 profile) as a documented academic-freedom interoperability choice where the book leaves wire details open | Coordinator project decision dated 2026-07-28; [OPTION_B_DECISION.md](OPTION_B_DECISION.md) | Current `0.2.4-proposed` bundle; conformance suite |
...
`0.2.4-proposed`, **UNFROZEN**, built as the role-neutral top-level
...
nonce-domain fixes, while `0.2.4-proposed` reconciles the simulator-v3.0.0 wire
profile with the book (Appendix F result outcomes, optional `min_center_intensity`,
optional `receive_control`). It supersedes all earlier proposed byte sets. Remaining
