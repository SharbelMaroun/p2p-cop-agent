...
| OB-001 | The project selects Option B (simulator-v3 profile) as a documented academic-freedom interoperability choice where the book leaves wire details open | Coordinator project decision dated 2026-07-28; [OPTION_B_DECISION.md](OPTION_B_DECISION.md) | Current `0.2.6-proposed` bundle; conformance suite |
...
`0.2.6-proposed`, **UNFROZEN**, built as the role-neutral top-level
...
wire profile with the book (see [OPTION_B_HANDOFF.md](OPTION_B_HANDOFF.md) for the
per-revision change log). It supersedes all earlier proposed byte sets. Remaining
