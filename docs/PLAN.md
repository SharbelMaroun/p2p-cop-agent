...
| M1.5 Option-B contract repair and conformance | IN PROGRESS | Option-B decision recorded, role-neutral `0.2.3-proposed` bundle, explicit per-run config inputs, public/secret nonce-domain separation, protocol/message schemas, hash vectors, and neutral conformance | Green conformance suite and published `0.2.3-proposed` handoff. **Not DONE**: `M1.5-13`, cross-repository acceptance of an exact bundle revision, is `BLOCKED` on the coordinator, and the bundle stays `UNFROZEN` until it lands. Corrected 2026-08-07 by `check_ledger_consistency.py` — this row read `DONE` here while that task was open in the TODO, which is precisely the `G-11` failure |
...
| M5 FastMCP runtime and resilience | IN PROGRESS | Server/client peer, negotiation, deadlines, idempotency, retry, watchdog, orchestrator gateway, subsystems, play loop, tunnel boundary | Two independent local processes complete a resilient game. **Done:** both adapters; negotiation and mismatch refusal; the private opponent-URL boundary; the declared phase machine; the turn loop, sub-game driver and mutual audit over a real socket between two OS processes; deadlines, bounded retry, idempotency and backpressure; the liveness watchdog with persist-then-shutdown; the Orchestrator gateway with five subsystem ports and an import-boundary guard; the log manager and deadline tracker; adversarial-peer and rejection handling; and (2026-08-02) the **autonomous play loop** that drives the mailbox, so a sub-game now plays with nothing fed in by hand. **Remaining:** the `serve` CLI (`M5-17e`) and the two-machine tunnel rehearsal (`M5-07c`, which also needs M8 screenshot evidence). *Row corrected 2026-08-02: it still listed the sub-game driver, mutual audit, deadlines/retry and watchdog as remaining, all four of which had shipped.* |
| M6 Scent, belief and private strategy | DONE | Multiplicative scent, Cop-local belief, deterministic strategy, optional private verbal layer | Legal deterministic behavior under observation/fallback tests. All 71 M6 rows closed; belief beats the blind baseline 96.7% to 26.7% on the paired measurement in `PRD_strategy.md`, and the scent model is hash-locked and offered for parity (`M6-15`). Row corrected 2026-08-07: it still read `DEFERRED` after the milestone finished |
| M7 Series orchestration, artifacts, gatekeeper and reporting | DONE | Six-sub-game series, validated artifacts, API gatekeeper, signed report delivery | One complete local series produces accepted audit artifacts. All M7 rows closed 2026-08-07 when the send-only Gmail consent was run and `[reporting].credential_path` was wired with its location and scope guards (`M7-15`, `M7-15a`). Row corrected the same day by `check_ledger_consistency.py` |
| M8 GUI, replay, interoperability and security hardening | DONE | Local-truth GUI, verified/tampered replay, unknown-opponent interop, security/failure hardening | Remote rehearsal and evidence screenshots pass. Closed 2026-08-09 by `M8-03c`, the last open row: a friendly, uncounted game was played in the Police role against group `amireman` across two machines over a Cloudflare tunnel (`match outcome: SURVIVAL after 35 step(s)`, artifacts in `games/friendly-amireman/`, `replay` re-verifying that log). Screenshot evidence was already in place — `assets/live-gui-belief-map.png`, `assets/replay-verified-ok.png`, `assets/replay-tampered.png`. Row corrected by `check_ledger_consistency.py`, which flagged `G-11` the moment `M8-03c` closed |
