...
| M5 FastMCP runtime and resilience | IN PROGRESS | Server/client peer, negotiation, deadlines, idempotency, retry, watchdog, tunnel boundary | Two independent local processes complete a resilient game. Done so far: both adapters, negotiation and mismatch refusal, the private opponent-URL boundary, the declared phase machine, one turn of the loop, and a negotiate round trip across a real socket between two OS processes. Remaining: sub-game driver, mutual audit, deadlines/retry, watchdog, tunnel |
