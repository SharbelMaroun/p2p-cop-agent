...
| 5 | Docstrings everywhere | 2 | Ruff `D` rules in the pinned select set; zero findings |
| 6 | Automated tests with meaningful coverage | 2 | 1573 tests, 97.28% branch against an 85% floor |
...
| 10 | No secrets in the repository | 2 | Tree and full history; 2363 objects, 1 reviewed false positive pinned by blob SHA, 0 unreviewed |
...
**Coverage is lower here than in the companion repository** — 97.28% against the Thief's
99.26%. Both clear the 85% floor, so no gate complains, but the gap is real and is in this
repository rather than that one.

**The companion agent cannot be launched.** `p2p-cop serve` plays a real match from the
command line; the Thief's CLI is a scaffold (`M9-025` there). A counted game needs both ends,
so the pair is not runnable end-to-end today even though this half is.
...
**26 / 30 (87%).** Four requirements at partial credit, none at zero. Recorded here rather
