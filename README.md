...
### The measurements were never measuring us (`C-052`, 2026-08-16)

Group `yanell11` told us our `smell_grid` should be the accumulated trail rather than a 5×5
window, and pointed at the reference source rather than arguing it. They were right: the field
persists across turns, `snapshot()` returns the whole thing, and `smell_grid_size` sizes the
emission *kernel*, not the transmitted dict. Our window was the deviation — from the profile
this repository itself adopted — and it made our field **harder for an opponent to invert than
the shape it was imitating**.

Fixing it exposed something larger. Eight capture-rate tests had been red since the scent clamp
landed, and the explanation — "the clamp degraded the belief" — was plausible, repeated all
evening, and wrong:

- every arena script fed a 5×5 **window**, so every capture-rate number published here was
  measured against a shape no peer sends;
- the belief arm ran `scent_likelihood`, the raw-intensity weighting replaced two milestones
  ago, not the `emitter_likelihood` the live path uses.

Measured properly — against a trail generated by the opponent kit's own reference
implementation — the decoder localises **16 of 16 turns**, against 12 of 16 from a window. Both
harnesses corrected, all eight failures cleared, and the clamp's measurable cost to the shipped
agent is **zero**.

The uncomfortable part is that this is a lesson already written down here. `M11-01` replaced a
decoder that was "grading its own homework"; its replacement reads a window shape that **only
our own emitter produces**, and fired 3 times in 102 observations against a real peer. Having
recorded the lesson did not prevent repeating it, because the new assumption looked like a fact
about the wire rather than a fact about us. The only cure that has actually worked is to
generate the test inputs from the other side's code.
